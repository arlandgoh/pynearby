...
# get_amenity_count(QUERY_RESTAURANT)
# print (get_subway_all())
# print (get_busstop_all())
latitude = '40.68513770564301'
longitude = '-73.95975749881096'
ts_latitude = '40.7580'
ts_longitude = '-73.9855'
...
def extractLocationFeatures(threadId=0, input_file_name=os.path.join(DATA_SET_DIRECTORY, FILE_NAME)):
    ...
    with open(input_file_name, encoding='utf-8') as csv_file:
        ...
        for row in csv_reader:
            ...
            #with open(output_file_name, mode='a', newline='') as new_csv_file:
            #    print('kk')
...
# logging.basicConfig(filename="log_file{0}.log".format(datetime.datetime.now()), format='%(asctime)s: %(levelname)s - %(message)s', datefmt='%d/%m/%Y %I:%M:%S %p', filemode='w')
...
# logger.info('Information for a Listing')
#logger.info('Subway: 200m = %s, 1000m = %s',
#            get_public_transport_count(latitude, longitude, OVERPASS_QUERY_SUBWAY, MIN_RADIUS),
#            get_public_transport_count(latitude, longitude, OVERPASS_QUERY_SUBWAY, MAX_RADIUS))
#logger.info('Subway Count: %s', get_subway_all())

threaded_location_extraction()
#pandasDFfromFile()
# print('Bus Stop', get_public_transport_count(latitude, longitude, OVERPASS_QUERY_BUS_STOP, MIN_RADIUS), get_public_transport_count(latitude, longitude, OVERPASS_QUERY_BUS_STOP, MAX_RADIUS))
# print(QUERY_RESTAURANT, get_amenity_count(latitude, longitude, QUERY_RESTAURANT, MIN_RADIUS), get_amenity_count(latitude, longitude, QUERY_RESTAURANT, MAX_RADIUS))
# print(QUERY_CAFE, get_amenity_count(latitude, longitude, QUERY_CAFE, MIN_RADIUS), get_amenity_count(latitude, longitude, QUERY_CAFE, MAX_RADIUS))
# print('Shops', get_shop_count(latitude, longitude, MIN_RADIUS), get_shop_count(latitude, longitude, MAX_RADIUS))
# print('Tourist Attractions', get_tourist_attractions_count(latitude, longitude, MIN_RADIUS), get_tourist_attractions_count(latitude, longitude, MAX_RADIUS))

# print ('Times Square')
# print('Subway', get_public_transport_count(ts_latitude, ts_longitude, OVERPASS_QUERY_SUBWAY, MIN_RADIUS), get_public_transport_count(ts_latitude, ts_longitude, OVERPASS_QUERY_SUBWAY,MAX_RADIUS))
# print('Bus Stop', get_public_transport_count(ts_latitude, ts_longitude, OVERPASS_QUERY_BUS_STOP, MIN_RADIUS), get_public_transport_count(ts_latitude, ts_longitude, OVERPASS_QUERY_BUS_STOP, MAX_RADIUS))
# print(QUERY_RESTAURANT, get_amenity_count(ts_latitude, ts_longitude, QUERY_RESTAURANT, MIN_RADIUS), get_amenity_count(ts_latitude, ts_longitude, QUERY_RESTAURANT, MAX_RADIUS))
# print(QUERY_CAFE, get_amenity_count(ts_latitude, ts_longitude, QUERY_CAFE, MIN_RADIUS), get_amenity_count(ts_latitude, ts_longitude, QUERY_CAFE, MAX_RADIUS))
# print('Shops', get_shop_count(ts_latitude, ts_longitude, MIN_RADIUS), get_shop_count(ts_latitude, ts_longitude, MAX_RADIUS))
# print('Tourist Attractions', get_tourist_attractions_count(ts_latitude, ts_longitude, MIN_RADIUS), get_tourist_attractions_count(ts_latitude, ts_longitude, MAX_RADIUS))
